...
> **Note**: 1つ1つの広告にVerifiable Credentialが付与される。そんな世界観をイメージしたデモ用アプリケーションです。バナー広告は広告主名のパターンマッチによるモック判定を行っています。サイトVCについては、HTMLに埋め込まれたSD-JWT形式のVCをDID/VC Engineでリアル検証します（Phase 7〜）。
...
- [x] **Phase 6**: VCギャラリーサイドパネル（ストーリーバー型マルチVC表示）
- [x] **Phase 7**: コンテンツ証明書VC埋め込み＋リアル検証（Verify API連携、全サイト対応）
