...
VeriCerts認証のコンセプトを実演するためのデモ用Chrome拡張機能です。SNSフィード上の広告を自動検出し、以下のように表示します：

- **フェイク広告（未認証）**: 警告オーバーレイを表示
- **認証済み広告**: VeriCerts認証バッジを表示

> **Note**: これはデモ用アプリケーションです。実際のVC（Verifiable Credentials）検証は行わず、広告主名のパターンマッチによるモック判定を行います。
