...
### リアルVC検証（Phase 7: サイトVC）
- `<script type="application/dc+sd-jwt">` でHTMLに埋め込まれたVCを自動検出
- DID/VC Engine（review環境）の `POST /v1/vc/verify` でリアル検証
- 署名、失効、フォーマット、発行者の検証結果をサイドパネルに表示
- `vc-detector.ts` が全サイトで動作（`<all_urls>`）
- バナー広告は引き続きモック判定

### 判定フロー
```
バナー広告検出（news-site.ts、デモサイトのみ）
  ↓
広告主名を取得
  ↓
...
VC検出（vc-detector.ts、全サイト）
  ↓
<script type="application/dc+sd-jwt"> を検出
  ↓
Background → Verify API呼び出し → サイドパネルに結果表示
```
...
2. **VC検証**: バナー広告はモック判定、サイトVCはDID/VC Engineでリアル検証（Phase 7〜）
3. **SNSのDOM依存**: 各SNSのDOM構造変更により動作しなくなる可能性があります
4. **公開リポジトリ**: このリポジトリはパブリックです。機密情報をコミットしないでください
5. **review環境依存**: サイトVCのリアル検証はreview環境（zero-engine-review.vericerts.io）に依存します
